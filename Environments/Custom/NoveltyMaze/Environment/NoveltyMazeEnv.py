import subprocess
import gym
from Environments.Custom.NoveltyMaze.Communication import CommunicationHandler, Message
from Environments.Custom.NoveltyMaze.Environment import GameState
import numpy as np
import time
import os

class NoveltyMazeEnv(gym.Env):
    def __init__(self, pipe_id=None, optimizer_id="backprop", discrete=True, step_rewards=True, symmetric=False):
        if pipe_id is None:
            pipe_id = os.getpid()
        ...
        if discrete:
            ...
            self.action_space = gym.spaces.Discrete(self.num_discrete_bins*self.num_discrete_bins)
        elif symmetric:
    ...
    def step(self, action):
        ...
        if state is None:
            ...
            print("ATTEMPTED TO TAKE ACTION {} WHEN STATE RECEIVE FAILED".format(action))
    ...
    def transmit_action(self, action):
        action_string = ''.join(["{}{}".format(arg, Message.NOVELTY_MAZE_MESSAGE_DATA_DELIMITER) for arg in action])

        exception = self.comm_handler.send_message(header=Message.NOVELTY_MAZE_AGENT_ACTION_IMMEDIATE_RESPONSE_MESSAGE_HEADER,
                                                   body = action_string)
    ...
    def save(self, filename, folder_path="H:/PyCharm/stanley_novelty_playbacks/in_progress"):
