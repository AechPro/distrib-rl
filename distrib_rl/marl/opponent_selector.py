...
class OpponentSelector(object):
    ...
    def get_opponent(self):
        decoded = self.client.get_data(redis_keys.MARL_CURRENT_OPPONENT_KEY)
        if decoded is None:
            return None, -1
        return decoded
    ...
    def update_opponent(self):
        if self.cfg["rng"].randint(0, 10) > 2 or len(self.player_skills) == 0:
            self.client.set_data(redis_keys.MARL_CURRENT_OPPONENT_KEY, (-1, -1))
            return
        ...
        self.client.set_data(
            redis_keys.MARL_CURRENT_OPPONENT_KEY, (params, opponent_num)
        )

    def update_ratings(self):
        results = self.client.atomic_pop_all(redis_keys.MARL_MATCH_RESULTS_KEY)
    ...
    def submit_result(self, opponent_num, victory):
        self.client.push_data(
            redis_keys.MARL_MATCH_RESULTS_KEY, (opponent_num, victory)
        )
