...
class Process(MPProcess):
    ...
    def __init__(
        self,
        process_name="unnamed_mpf_process",
        loop_wait_period=None,
        process_all_updates=False,
    ):
        super(MPProcess, self).__init__()
        self._out = None
        self._inp = None
    ...
    def run(self):
        ...
        try:
            ...
            from distrib_rl.mpframework import ResultPublisher
            from distrib_rl.mpframework import TaskChecker
            import logging
            import time

            # This are our i/o objects for interfacing with the main process.
            self.task_checker = TaskChecker(self._inp, self.name)
            self.results_publisher = ResultPublisher(self._out, self.name)

            self._logger = logging.getLogger("MPFLogger")
            self._logger.debug("MPFProcess initializing...")

            # Initialize.
            self.init()
            self._logger.debug(
                "MPFProcess {} has successfully initialized".format(self.name)
            )
            ...
            while True:
                ...
                while self.task_checker.check_for_update():
                    self._logger.debug(
                        "MPFProcess {} got update {}".format(
                            self.name, self.task_checker.header
                        )
                    )

                    # If we are told to stop running, do so.
                    if self.task_checker.header == Process.STOP_KEYWORD:
                        self._logger.debug(
                            "MPFPROCESS {} RECEIVED STOP SIGNAL!".format(self.name)
                        )
                    ...
                    self._logger.debug(
                        "MPFProcess {} sending update to subclass".format(self.name)
                    )
            ...
            self._logger.debug(
                "MPFProcess {} is exiting due to KeyboardInterrupt\n".format(self.name)
            )
            raise sys.exit(0)
            ...
            if self.task_checker is not None:
                self._logger.debug(
                    "MPFProcess {} Cleaning task checker...".format(self.name)
                )
                self.task_checker.cleanup()
                del self.task_checker
                self._logger.debug(
                    "MPFProcess {} has cleaned its task checker!".format(self.name)
                )

            if self.results_publisher is not None:
                del self.results_publisher

            self._logger.debug("MPFProcess {} Cleaning up...".format(self.name))

            self._logger.debug("MPFProcess {} Exiting!".format(self.name))
            return
