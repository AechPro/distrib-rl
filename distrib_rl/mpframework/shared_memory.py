"""
    ...
    Description:
        ...
        asynchronously by multiple Process objects. This is purposefully not thread safe because its intended usage
...
class SharedMemory(object):
    ...
    def _allocate(self):
        ...
        self._logger.debug(
            "Allocating MPFMemoryBlock!\nSize: {}\nData type: {}.".format(
                self._size, self.dtype
            )
        )
        ...
        self._memory = self._manager.MPFSharedMemoryBlock(
            self._size, self.dtype, rng=self.rng
        )
        self._logger.debug("MPFMemoryBlock allocated successfully!")

    def cleanup(self):
        try:
            self._logger.debug("Cleaning up MPFMemoryBlock...")

            self._memory.cleanup()
            self._logger.debug("Shutting down MPFMemory manager...")

            self._manager.shutdown()
            self._logger.debug("MPFMemoryBlock has closed successfully!")

        except:
            import traceback

            self._logger.critical(
                "MPFMemoryBlock was unable to close!"
...
class MPFSharedMemoryBlock(object):
    ...
    def _parse_dtype(self, code):
        ...
        if code in float_codes:
            return SharedMemory.MPF_FLOAT32

        if code in double_codes:
            return SharedMemory.MPF_FLOAT64

        if code in int_codes:
            return SharedMemory.MPF_INT32

        if code in long_codes:
            return SharedMemory.MPF_INT64
