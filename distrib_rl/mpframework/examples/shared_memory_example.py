"""
    File name: shared_memory_example.py
    Author: Matthew Allen

    Description:
        This file contains an example for using a shared memory block with a Process.
...
from distrib_rl.mpframework import Process, ProcessHandler, SharedMemory
...
def main():
    ...
    size = 1000
    dtype = SharedMemory.MPF_INT32  # These are just references to ctypes data types.

    # Ask MPFramework to allocate some memory for us. Here we have requested 1,000 entries of data type int32.
    memory_container = SharedMemory(size, dtype=dtype)
