from distrib_rl.policy_optimization.policy_gradients import configurator
...
class PolicyGradients(object):
    ...
    def train(self):
        ...
        while ts_consumed < int(self.cfg["policy_optimizer"]["max_timesteps"]):
            ...
            self.epoch_info["update_magnitude"] = np.linalg.norm(
                np.subtract(before, after)
            )
    ...
    def collect(self):
        ...
        for trajectory in trajectories:
            values = (
                value_estimator.get_output(trajectory.obs).flatten().numpy().tolist()
            )
            final_val = (
                value_estimator.get_output(trajectory.next_obs[-1]).numpy().tolist()
            )
    ...
    def update_models(self):
        ...
        self.epoch_info["ts_consumed"] = (
            num_batches * self.cfg["policy_optimizer"]["batch_size"]
        )
