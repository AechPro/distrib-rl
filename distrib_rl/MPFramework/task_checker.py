"""
    File name: task_checker.py
    Author: Matthew Allen

    Description:
        This file handles checking the input to an Process object. It handles reading from the input queue
        ...
        simply check the latest_data and header variables inside the TaskChecker object.
"""
...
class TaskChecker(object):
    ...
    def __init__(
        self, input_queue, name, update_check_sleep_period=0.1, init_sleep_period=1.0
    ):
    ...
    def wait_for_initialization(self, header=None):
        ...
        self._MPFLog.debug(
            "Process {} is waiting for initialization...".format(self._name)
        )
        ...
        if header is not None and self.header is not None:
            if self.header == header:
                self._MPFLog.debug("Process {} has initialized!".format(self._name))
                return
    ...
    def check_for_update(self):
        ...
        Process run loop. This function checks for data and processes it into self.header and self.latest_data
        for access by an Process object.
        ...
        if not self._input_queue.empty():
            ...
            self._MPFLog.debug(
                "Process {} has received a new data packet!".format(self._name)
            )
    ...
    def cleanup(self):
        try:
            ...
            try:
                fail_count = 0
                # This is a copy and paste of get_all() from ProcessHandler.
                # TODO: fixme
                ...
                self._MPFLog.critical(
                    "GET_ALL ERROR FROM {} MPF TASK CHECKER!\n{}".format(
                        self._name, error
                    )
                )
            finally:
    ...
    def _check_for_terminal_message(self, header, data):
        ...
        for word in self.EXIT_KEYWORDS:
            if word in h:
                self.header = Process.MPFProcess.STOP_KEYWORD
                self._MPFLog.debug(
                    "Process {} has received a terminate command!".format(self._name)
                )
                return
