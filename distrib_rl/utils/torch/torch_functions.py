...
class MultiDiscreteSB3(nn.Module):
    ...
    def make_distribution(self, policy_output):
        ...
        self.distribution = [
            distribs.Categorical(logits=split)
            for split in torch.split(policy_output, tuple(self.bins), dim=self.dim)
        ]
        ...
        return torch.stack(
            [
                ...
                for dist, action in zip(
                    self.distribution, torch.unbind(actions, dim=self.dim)
                )
            ],
            dim=self.dim,
        ).sum(dim=self.dim)
    ...
    def entropy(self) -> torch.Tensor:
        return torch.stack(
            [dist.entropy() for dist in self.distribution], dim=self.dim
        ).sum(dim=self.dim)
