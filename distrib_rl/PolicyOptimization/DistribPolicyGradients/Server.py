...
class Server(object):
    ...
    def step(self):
        ...
        if self.epoch_info["ts_consumed"] > 0:
            self.server.redis.set(
                RedisKeys.SERVER_CUMULATIVE_TIMESTEPS_KEY, self.cumulative_ts
            )
            self.server.redis.set(
                RedisKeys.RUNNING_REWARD_MEAN_KEY, float(self.exp_manager.rew_mean)
            )
            self.server.redis.set(
                RedisKeys.RUNNING_REWARD_STD_KEY, float(self.exp_manager.rew_std)
            )
    ...
    def setup_redis(self, cfg):
        ...
        self.server.redis.set(
            RedisKeys.SERVER_CURRENT_STATUS_KEY, RedisServer.RECONFIGURE_STATUS
        )
    ...
    def reset(self):
        ...
        self.server.redis.set(
            RedisKeys.SERVER_CURRENT_STATUS_KEY, RedisServer.RESET_STATUS
        )
